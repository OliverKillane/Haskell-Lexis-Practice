...
lookUpAtt nm hd rw
  = lookUp nm $ zipWith (\(x,_) y -> (x,y)) hd rw

removeAtt :: AttName -> Header -> Row -> Row
removeAtt nm hd rw
  = map snd $ remove nm $ zipWith (\(x,_) y -> (x,y)) hd rw

addToMapping :: Eq a => (a, b) -> [(a, [b])] -> [(a, [b])]
addToMapping (x,v) [] = [(x,[v])]
addToMapping e@(x,v) (m@(x', vs):ms)
    | x' == x = (x, v:vs):ms
    | otherwise = m:addToMapping e ms

buildFrequencyTable :: Attribute -> DataSet -> [(AttValue, Int)]
--Pre: Each row of the data set contains an instance of the attribute
buildFrequencyTable (nm, vs) (hd, rs) 
    = zip vs $ map (\x -> length $ filter(==x) (map (lookUpAtt nm hd) rs)) vs
